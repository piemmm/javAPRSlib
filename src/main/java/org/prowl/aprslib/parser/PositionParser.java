...
//    }
    ...
    public static Position parseNMEA(byte[] msgBody) throws Exception {
        ...
        if ("$GPGGA".equals(nmea[0]) && nmea.length >= 15) {
            ...
            //$GPGGA,000045.996,0340.0000,N,02345.0000,E,0,00,50.0,0.0,M,,,,0000*3C

            // This library shouldn't involve itself with refusing fixes based on reported quality.
//            if (!("1".equals(nmea[6]))) { // Not valid position fix
//                throw new UnparsablePositionException("Not a valid position fix:"+new String(msgBody));
//            }
            ...
            // $GPGLL,4609.105,N,12258.824,W,122358,A*3A  (this is also valid)
        ...
        if (lats == null) {
            throw new UnparsablePositionException("Invalid NMEA sentence:" + new String(msgBody));
        }
        try {
